...
export const Route = createFileRoute("/_authenticated/marketplace/automation")({
  head: () => ({ meta: [{ title: "Автоматизация · ЦифроРубль" }] }),
  component: Page,
});
...
function Page() {
  return (
    ...
    >
      <FeatureGrid items={[
          { icon: Sparkles, title: "Триггеры", description: "События платформы" },
          { icon: Sparkles, title: "Условия", description: "Логика" },
          { icon: Sparkles, title: "Действия", description: "API вызовы" },
          { icon: Sparkles, title: "AI", description: "Шаги с AI" },
          { icon: Sparkles, title: "Webhooks", description: "Внешние вызовы" },
          { icon: Sparkles, title: "Уведомления", description: "Email/Push" }
        ]} />
    </PlatformPage>
  );
}
