...
export const Route = createFileRoute("/_authenticated/admin/teams")({
  head: () => ({ meta: [{ title: "Команды · Администрирование" }] }),
  component: Page,
});
...
function Page() {
  return (
    <PlatformPage
      title="Команды"
      description="Подразделения, отделы, руководители, приглашения и роли сотрудников"
      integration={<IntegrationStatus />}
      action={<Button size="sm"><Plus className="mr-1.5 h-3.5 w-3.5" />Создать</Button>}
    >
      <EmptyState
        title="Раздел готов к работе"
        description="Интерфейс модуля «Команды» подключается к ядру платформы через официальные API и политику разграничения прав."
      />
    </PlatformPage>
  );
}
