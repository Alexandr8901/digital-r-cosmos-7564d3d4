...
export const Route = createFileRoute("/_authenticated/admin/api")({
  head: () => ({ meta: [{ title: "API · Администрирование" }] }),
  component: Page,
});
...
function Page() {
  return (
    <PlatformPage
      title="API"
      description="Управление публичным API: ключи, лимиты, версии и документация"
      integration={<IntegrationStatus />}
      action={<Button size="sm"><Plus className="mr-1.5 h-3.5 w-3.5" />Создать</Button>}
    >
      <EmptyState
        title="Раздел готов к работе"
        description="Интерфейс модуля «API» подключается к ядру платформы через официальные API и политику разграничения прав."
      />
    </PlatformPage>
  );
}
