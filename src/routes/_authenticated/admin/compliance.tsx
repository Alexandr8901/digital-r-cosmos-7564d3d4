...
export const Route = createFileRoute("/_authenticated/admin/compliance")({
  head: () => ({ meta: [{ title: "Соответствие · Администрирование" }] }),
  component: Page,
});
...
function Page() {
  return (
    <PlatformPage
      title="Соответствие"
      description="Политики, согласия, журналы обработки данных и хранение"
      integration={<IntegrationStatus />}
      action={<Button size="sm"><Plus className="mr-1.5 h-3.5 w-3.5" />Создать</Button>}
    >
      <EmptyState
        title="Раздел готов к работе"
        description="Интерфейс модуля «Соответствие» подключается к ядру платформы через официальные API и политику разграничения прав."
      />
    </PlatformPage>
  );
}
