...
export const Route = createFileRoute("/_authenticated/finance/bank-accounts")({
  head: () => ({ meta: [{ title: "Банковские счета · ЦифроРубль" }] }),
  component: Page,
});
...
function Page() {
  return (
    <PlatformPage
      ...
      description="Подключённые счета через Open API банков"
      action={
        <>
    ...
    >
      <FeatureGrid items={[
          { icon: Sparkles, title: "Сбербанк", description: "Подключение через банк-партнёр" },
          { icon: Sparkles, title: "Альфа-Банк", description: "Open API" },
          { icon: Sparkles, title: "Т-Банк", description: "Open API" },
          { icon: Sparkles, title: "Газпромбанк", description: "Подключение через банк-партнёр" }
        ]} />
    </PlatformPage>
  );
}
