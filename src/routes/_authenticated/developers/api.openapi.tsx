...
export const Route = createFileRoute("/_authenticated/developers/api/openapi")({
  head: () => ({ meta: [{ title: "OpenAPI · ЦифроРубль" }] }),
  component: Page,
});
...
function Page() {
  return (
    <PlatformPage
      title="OpenAPI"
      description="Swagger, JSON, YAML, версии"
      action={
        <>
    ...
    >
      <FeatureGrid items={[
          { icon: Sparkles, title: "openapi.json", description: "Спецификация" },
          { icon: Sparkles, title: "openapi.yaml", description: "YAML" },
          { icon: Sparkles, title: "Swagger UI", description: "Интерактивный просмотр" },
          { icon: Sparkles, title: "Версии", description: "v1, v2" }
        ]} />
    </PlatformPage>
  );
}
